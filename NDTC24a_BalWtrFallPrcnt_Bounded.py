...
def getBadCellsValueRange( srcCells_Val, srcCells_Name, thresh, bounds, j ):
    ...
    if ( len(srcCells_Val) > 0 ):
        ...
        [ np.array(i)[ np.argmin( abs(np.array(i)) ) ] for i in zip(a-2, a-3)]
    else:
...
def getDestCellsAdd( i, j, k, numProducts=0, totalFields=0, numBalance=0, \
                     destScenIndex=0, balanceInd=[], spreadFieldsInd=[] ):
...
## FUNCTION TO GET INDICES AND COUNT OF SPECIFIC BALANCE FIELDS
def getSpecFieldsIndCount( specFields, fieldsList ):
    """ Returns the indices and count of Specific Balance Fields"""
    indices = []
    for j in range( 0, len(specFields) ):
        indices.extend( [i for i, item in enumerate( fieldsList) \
                        if specFields[j] in item.lower() and '%' in item.lower()] )
    numfields = len( specFields )
    return indices, numFields
...
def main( destSheet, testTitle, totalFields, numRates, numBalance,\
    ...
    ## SPECIFIC BALANCE FIELDS: Get Indices and Count of specific Balance fields
    specFieldsInd, numFileds = getSpecFieldsIndCount( specFields, fieldsList )
    ...
    for k in range( kstart, numScenarios+1):
        ...
        for i in range(1, numProducts+1):
            ...
            for j in range(1, numFields+1):
                ...
                Cell( destSheet, destCells_Rate.name ).value = balanceList[specBalInd[j-1]]
                Cell( destSheet, destCells_Rate.name ).font.color = balanceColor[j%2]
...
valsCol = 6
destSheet = 'NDTC24a'
testTitle = "Balance Waterfall Entries as a Percentage of Beg. Bal. satisfy "\
            "Min. and Max. Bounds"
ignoreRate = 'Rate Discretion'
scenarioOnly = 0  #1 for Scenario only, 0 for Both Scenario and Baseline
thresh = 15
specFields = ['acqusition', 'augmentation', 'transitions in', 'transitions out',\
               'attrition', 'interest payment', 'ending balance']
bounds = [0, 10, -10, 10, 0, 10, -10, 0, -10, 0, 0, 3, 90, 110]

main( destSheet, testTitle, totalFields, numRates, numBalance,\
