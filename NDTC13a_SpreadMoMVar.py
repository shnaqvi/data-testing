...
def convertSourceToFloat( srcCells_Val ):
    ''' Returns the Source Cells converted to Float type from Unicode'''
    if ( len(srcCells_Val) > 0 ):
        if ( type( srcCells_Val[0] ) == unicode ):  
            srcCells_Val = array( [ float( re.sub('\D', '', a) ) \
...
def main( destSheet, testTitle, numRates, \
    ...
    for k in range( kstart, numScenarios+1):
        ...
        for i in range(1, numProducts+1):
            ...
            ## if j is not used then set it to -1
            ...
            Cell( destSheet, destCells_Rate.name ).value = fieldsList[spreadFieldsInd[2]]
...
valsCol = 6
destSheet = 'NDTC13a'
testTitle = " Spread should not have a monthly relative percentage variation "\
            "of more than +/-0.5%"
ignoreRate = 'Rate Discretion'
scenarioOnly = 0  #1 for Scenario only, 0 for Both Scenario and Baseline
thresh = .5

main( destSheet, testTitle, numRates, \
