...
def getSrcCellsValueRange( mainSheet, totalFields, valsCol, i, j=0, k=0, \
                           numProducts=0, numRates=0, specBalInd=[], \
                           spreadFieldsInd=[] ):
    ...
    # Get baseline and scenario values for the given balance field
    for row in range(0,2):
        src_RowIndices[row]= row*(numProducts*totalFields) + ((i-1)*totalFields) + \
                             numRates + (specBalInd[j-1]+1) + 1
...
def getBadCellsValueRange( srcCells_Val, srcCells_Name, thresh, bpsVar):
    ...
    if ( len(srcCells_Val) > 0 ):
        ...
        var = (abs(srcCells_Val[1]) - abs(srcCells_Val[0]))
        var = np.array([ round(num, 1) for num in var])

        if ( bpsVar>0 ):
            badCells_Var = var[ var>0 ].tolist()
            badCells_Val = srcCells_Val[1][ var>0].tolist()
            badCells_Name = srcCells_Name[ var>0 ].tolist()
        else:
            badCells_Var = var[ var<0 ].tolist()
            badCells_Val = srcCells_Val[1][ var<0 ].tolist()
            badCells_Name = srcCells_Name[ var<0 ].tolist()
...
def getSpecBalanceIndCount( specBalance, balanceList ):
    """ Returns the indices and count of Specific Balance Fields"""
    indices = []
    for j in range( 0, len(specBalance) ):
        indices.extend( [ i for i, item in enumerate( balanceList)
                             if specBalance[j] in item.lower() ] )
    numBalance = len( specBalance )
    return indices, numBalance
...
def main( destSheet, testTitle, numRates, numBalance,\
    ...
    specBalInd, numBalance = getSpecBalanceIndCount( specBalance, balanceList )
...
valsCol = 6
destSheet = 'NDTC5a'
testTitle = "Attrition for the products, whose Pub. Rate is Dec./Inc-reased "\
            "by 25bps, should be Inc./Dec-reased respectively in Scenario "\
            "compared to the Baseline"
ignoreRate = 'Rate Discretion'
scenarioOnly = 1  #MUST BE 1 FOR COMPARING SCENARIO W/ BASELINE: 1 for Scenario 
thresh = 0
bpsVar = 10
specBalance = ['attrition']

main( destSheet, testTitle, numRates, numBalance,\
      kpisCol, prodCol, valsCol, ignoreRate, scenarioOnly, thresh, bpsVar, specBalance)
