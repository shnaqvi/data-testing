...
def getBadCellsValueRange( srcCells_Val, srcCells_Name, thresh):
    ...
    if ( len(srcCells_Val) > 0 ):
        # CRITERION #
        srcCells_Val_New = ( srcCells_Val[1]- np.sum( srcCells_Val[2:len(srcCells_Val)], axis=0) \
                - srcCells_Val[0] )
        var = np.true_divide(srcCells_Val_New, abs( srcCells_Val[0] ))* 100.
        var = np.array([ round(num, 2) for num in var])
        
        badCells_Var = var[ abs(var)> thresh ].tolist()
        badCells_Val = ( srcCells_Val_New[ abs(var)> thresh ] * 100 ).tolist()
        badCells_Name = srcCells_Name[0][ abs(var)> thresh ].tolist()
    else:
...
def ndtc7( destSheet, testTitle, numRates, \
    ...
    for k in range( kstart, numScenarios+1):
        ...
        for i in range(1, numProducts+1):
            ...
            Cell( destSheet, destCells_Field.name ).value = fieldsList[specFieldsInd[0]] +'_Calc - Spread_Actual'
...
if __name__ == '__main__':
    ...
    destSheet = 'NDTC7a'
    testTitle = ''' Spread Calculation is Correct: Base FTP - Published Rate = Spread; OR ''' \
                ''' Total FTP - Published Rate - Discretion = Spread; '''
    ...
    ndtc7( destSheet, testTitle, numRates, \
