...
def getBadCellsValueRange( srcCells_Val, srcCells_Name):
    ...
    if ( len(srcCells_Val) > 0 ):
        # CRITERION #
        badCells_Val = (srcCells_Val[ srcCells_Val < 0 ]*100 ).tolist() 
        badCells_Name = srcCells_Name[ srcCells_Val < 0 ].tolist()
    else:
        badCells_Val = badCells_Name = []
    return badCells_Val, badCells_Name
...
def getDestCellsAdd( i, j, k, numProducts=0, totalFields=0, destScenIndex=0):
    ...
    dest_RowIndex = ((destScenIndex-1)*(numProducts*2))+ ((i-1)*2) + 1 + 1
...
def ndtc3( destSheet, testTitle, numRates, \
          kpisCol, prodsCol, valsCol, ignoreRate, scenarioOnly, specFields):
    ...
    for k in range( kstart, numScenarios+1):
        ...
        for i in range(1, numProducts+1):
            ...
            Cell( destSheet, destCells_Field.name ).value = fieldsList[spreadInd]
...
if __name__ == '__main__':
    ...
    destSheet = 'NDTC3a'
    testTitle = 'Spread is strictly positive for all products under Baseline & Scenario'
    ignoreRate = 'Rate Discretion'
    scenarioOnly = 0  #1 for Scenario only, 0 for Both Scenario and Baseline
    specFields = ['spread', 'ftp', 'published.*rate$', 'discretion']

    ndtc3( destSheet, testTitle, numRates, \
          kpisCol, prodCol, valsCol, ignoreRate, scenarioOnly, specFields)
