...
def getSrcCellsValueRange( mainSheet, totalFields, valsCol, i, j=0, k=0, \
                           numProducts=0, numRates=0, spreadFieldsInd=[] ):
    ''' Returns lists of Values and Names of the Source cells
        for the given i and j indices '''
    src_RowIndex = ((k-1)*numProducts*totalFields) + ((i-1)*totalFields) + numRates + j + 1
...
def ndtc10( destSheet, testTitle, numRates, numBalance,\
    ...
    for k in range( kstart, numScenarios+1):
        ...
        for i in range(1, numProducts+1):
            ...
            ## FOR EACH OF THE BALANCE' ATTRIBUTES:
            for j in range(1, numBalance+1):
                ...
                Cell( destSheet, destCells_Rate.name ).value = balanceList[j-1]
...
if __name__ == '__main__':
    ...
    destSheet = 'NDTC10a'
    testTitle = "All Balance WaterFall entries, including Acqu., Augm, Attr, " \
                "Trans., etc., balances shall not have Monthly Variation > +/-10%" 
    ignoreRate = 'Rate Discretion'
    scenarioOnly = 0  #1 for Scenario only, 0 for Both Scenario and Baseline
    thresh = 15

    ndtc10( destSheet, testTitle, numRates, numBalance,\
