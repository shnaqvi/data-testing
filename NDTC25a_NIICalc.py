...
def getBadCellsValueRange( srcCells_Val, srcCells_Name, thresh):
    ...
    if ( len(srcCells_Val) > 0 ):
        srcCells_Val_New = (srcCells_Val[0]/12 * \
                            (srcCells_Val[1]+ srcCells_Val[2])/2)- srcCells_Val[3] 
        # CRITERION #
        var = np.true_divide( srcCells_Val_New, abs( srcCells_Val[3] )) * 100.
...
def ndtc25( destSheet, testTitle, numRates, \
    ...
    for k in range( kstart, numScenarios+1):
        ...
        for i in range(1, numProducts+1):
            ...
            Cell( destSheet, destCells_Field.name ).value = "NII-Spread*AvgBal"
...
if __name__ == '__main__':
    ...
    destSheet = 'NDTC25a'
    testTitle = "NII Calculation is Correct upto a ballpark threshold: "\
                "Spread/12 * (Beg.Bal+ End.Bal)/2 = NII"
    ignoreRate = 'Rate Discretion'
    scenarioOnly = 0  #1 for Scenario only, 0 for Both Scenario and Baseline
    thresh = 10
    specFields = ['spread', 'beg.*balance$', 'end.*balance$', \
                  'nii|net interest income']

    ndtc25( destSheet, testTitle, numRates, \
