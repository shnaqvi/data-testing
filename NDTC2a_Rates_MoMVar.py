...
def getBadCellsValueRange( srcCells_Val, srcCells_Name, upperThresh, lowerThresh):
    ...
    if ( len(srcCells_Val) > 0 ):
        ...
        # CRITERIA
        badCells_Var = var[ (var > upperThresh) | (var < -lowerThresh) ].tolist()
        badCells_Val = srcCells_Val_New[ (var > upperThresh) | (var < -lowerThresh) ]*100
        badCells_Val = badCells_Val.tolist()
        
        badCells_Name = srcCells_Name_New[ (var>upperThresh) | (var<-lowerThresh)].tolist()
    else:
...
# FUNCTION TO GET NAMES AND COUNTS OF ALL FIELDS:
...
def getThresholdFromSheet( destSheet, upperThresh, lowerThresh ):
    ...
    if destSheet in all_sheets():
        if Cell( destSheet, 2, 2).value != None:
            lowerThresh = Cell( destSheet, 2, 2).value
        if Cell( destSheet, 2, 4).value != None:
            upperThresh = Cell( destSheet, 2, 4).value
    return upperThresh, lowerThresh

# FUNCTION TO ENTER HOUSE-KEEPING DATA ONTO THE WORKSHEET
def printTitleAndThreshold( destSheet, testTile, upperThresh=None, lowerThresh=None):
    ...
    if upperThresh != None and lowerThresh != None:
        Cell(destSheet, 2, 1).value = 'L. Thresh:'
        Cell(destSheet, 2, 1).font.bold = True

        Cell(destSheet, 2, 2).value = lowerThresh
        Cell(destSheet, 2, 2).color = 'yellow'

        Cell(destSheet, 2, 3).value = 'U. Thresh:'
        Cell(destSheet, 2, 3).font.bold = True

        Cell(destSheet, 2, 4).value = upperThresh
...
def ndtc2( destSheet, testTitle, upperThresh, lowerThresh, numRates, \
    ...
    upperThresh, lowerThresh = getThresholdFromSheet( destSheet,\
    ...
    for k in range( kstart, numScenarios+1):
        ...
        for i in range(1, numProducts+1):
            ...
            for j in range(1, numRates+1):
                ...
                printTitleAndThreshold( destSheet, testTitle, upperThresh, lowerThresh )
                ...
                if ( ignoreRate not in ratesList[j-1] ):
...
if __name__ == '__main__':
    ...
    destSheet = 'NDTC2a'
    testTitle = " Rates' Month to Month Variation is Not Greater than " \
                "the Thresholds under Baseline and Scenario"
    ignoreRate = 'xxx' #'Rate Discretion'
    scenarioOnly = 0  #1 for Scenario only, 0 for Both Scenario and Baseline
    upperThresh = lowerThresh = 10

    ndtc2( destSheet, testTitle, upperThresh, lowerThresh, numRates, \
          scenCol, prodCol, kpisCol, valsCol, ignoreRate, scenarioOnly)
