...
def getSrcCellsValueRange( mainSheet, totalFields, valsCol, i, j=0, k=0, \
    ...
    src_RowIndex = ((k-1)*numProducts*totalFields) + ((i-1)*totalFields) + numRates + \
                   (begEndBalInd[1]+1) + 1
...
valsCol = 6
destSheet = 'NDTC8a'
testTitle = "Ending Balance Inc/Dec-rease respectively Monthly in Scenario for "\
            "all products whose Pub. Rate has been Inc/Dec-reased by 20bps" 
ignoreRate = 'Rate Discretion'
scenarioOnly = 1  #1 for Scenario only, 0 for Both Scenario and Baseline
thresh = 1
bpsVar = 20
