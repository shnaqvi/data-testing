...
def getSrcCellsValueRange( mainSheet, totalFields, valsCol, i, j=0, k=0, \
    ...
    src_RowIndex = ((k-1)*numProducts*totalFields)* np.array(len(specFieldsInd)*[1]) \
                   ...
                   + np.array(specFieldsInd) \
                   + np.array( len(specFieldsInd)* [1+1] )
    #src_RowIndex = ( ((k-1)*numProducts*totalFields)+ ((i-1)*totalFields)\
    #                 + numRates)* np.array( len(specFieldsInd)* [1] )\
    #                + np.array( specFieldsInd) + np.array( len(specFieldsInd)* [1+1] )

    #print specFieldsInd
    #print src_RowIndex
    temp = Cell( mainSheet, np.asscalar( src_RowIndex[0] ), \
                                         valsCol ).horizontal
    ...
    for row in range( 0, len(src_RowIndex) ):
        temp_Val = np.array( Cell( mainSheet, np.asscalar( src_RowIndex[row]), \
                                            valsCol ).horizontal )
...
def getBadCellsValueRange( srcCells_Val, srcCells_Name, thresh):
    ...
    if ( len(srcCells_Val) > 0 ):
        ...
        srcCells_Val_NextMo = srcCells_Val[0][1:]
        srcCells_Val_LastMo = srcCells_Val[1][:len(srcCells_Val[1])-1]
        srcCells_Val_New = srcCells_Val_NextMo- srcCells_Val_LastMo
        srcCells_Name_New = srcCells_Name[0][1:]
        
        # CRITERION # (division by 0 in float results in float(inf))
        var = np.true_divide( srcCells_Val_New, abs(srcCells_Val_LastMo) ) * 100
...
def ndtc28( destSheet, testTitle, numRates, numBalance,\
    ...
    for k in range( kstart, numScenarios+1):
        ...
        for i in range(1, numProducts+1):
            ...
            destCells_Scen, destCells_Prod, destCells_Field, destCells_Stat, \
                    destCells_Add, destCells_Val, destCells_Var = \
                    getDestCellsAdd( i, 0, 0, numProducts, 0, destScenIndex)
            ...
            Cell( destSheet, destCells_Field.name ).value = "BegBal- EndBal"
            Cell( destSheet, destCells_Field.name ).font.color = balanceColor[1]
...
if __name__ == '__main__':
    ...
    destSheet = 'NDTC28a'
    testTitle = "Beginning Balance is within +/-2.5% of previous month's "\
                "Ending Balance for all products in both Baseline and Scenario"
    ...
    ndtc28( destSheet, testTitle, numRates, numBalance,\
